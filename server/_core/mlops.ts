...
export class MLOpsService {
  ...
  static async createModelVersion(
    data: z.infer<typeof createModelVersionSchema>,
    userId: number
  ) {
    ...
    const result = await db
      ...
      .values({
        ...
        hyperparameters: validatedData.hyperparameters
          ? JSON.stringify(validatedData.hyperparameters)
          : null,
        createdBy: userId,
      })
      .returning();
  ...
  static async updateExperimentMetrics(
    experimentId: number,
  ...
  static async updatePredictionFeedback(
    predictionId: number,
    actualOutcome: any,
    feedback: string,
    reviewedBy: number
  ...
  static async recordModelMetric(
  ...
  static async getDriftAlerts(modelVersionId?: number) {
    let query = db
      ...
      .where(eq(modelDriftMonitoring.isAlert, true));

    if (modelVersionId) {
      query = query.where(
        eq(modelDriftMonitoring.modelVersionId, modelVersionId)
      );
    }
  ...
  static async getAnomalies(
    ...
    if (filters.detectionType) {
      query = query.where(
        eq(anomalyDetections.detectionType, filters.detectionType)
      );
    }
    if (filters.severity) {
...
export class FeatureValidator {
  ...
  static validateFeatures(
    ...
    for (const featureDef of featureDefinitions) {
      ...
      const rules = featureDef.validationRules
        ? JSON.parse(featureDef.validationRules)
        : {};
      ...
      if (value !== undefined && value !== null) {
        ...
        if (!isValid) {
          errors.push(
            `Feature '${featureDef.featureName}' tem valor inválido: ${value}`
          );
        }
      }
    }
